...
def process_segmented_csv(csv_file, output_file):
    ...
    # Feature extraction
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Processing segmented audio files"):
        if os.path.exists(row['segment_file']):
            try:
                features = extract_audio_features_with_praat(row['segment_file'])
                # Add additional metadata from the row
                features.update({
                ...
                print(f"Error processing {row['segment_file']}: {e}")

    # Create DataFrame and save to CSV
    result_df = pd.DataFrame(results)
    result_df.to_csv(output_file, index=False)
    print(f"Audio features extracted and saved to {output_file}")
    
    return result_df

def main():
    parser = argparse.ArgumentParser(description="Extract audio features from segmented audio files.")
    parser.add_argument('--input-csv', required=True, help='CSV file containing segmented audio file paths')
    parser.add_argument('--output-csv', required=True, help='Output CSV file for extracted features')
    args = parser.parse_args()

    # Call the function to process segmented CSV
    process_segmented_csv(args.input_csv, args.output_csv)
