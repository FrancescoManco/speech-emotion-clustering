...
def standardize_and_process_df(df, thresholds, stats, num_classes):
    ...
    for feature in features:
        df[f'{feature}_standardized'] = df.apply(lambda row:
            (row[feature] - stats['overall'][feature]['mean']) /
            stats['overall'][feature]['std']
...
def process_audio_features(input_csv, output_csv, num_classes=5):
    """ Process audio features from a CSV file, generate descriptions and impressions, and save the results.
    Args:
        input_csv (str): CSV path with extracted audio features
        output_csv (str): CSV path to save processed features
        num_classes (int): Number of classes for the categorization (3, 4, 5, 6)
    Returns:
        pd.DataFrame: DataFrame with processed audio features, descriptions, and impressions
    """
    # Load the dataset
    df = pd.read_csv(input_csv)

    # Extract thresholds and stats from the entire dataset
...
def main():
    parser = argparse.ArgumentParser(description="Process audio features and generate descriptions")
    parser.add_argument('--input-csv', required=True, help='CSV file with extracted audio features')
    parser.add_argument('--output-csv', required=True, help='Output CSV file with processed features')
    parser.add_argument('--num-classes', type=int, default=5, choices=[3,4,5,6], help='Number of classes for categorization (default: 5)')
    args = parser.parse_args()

    # Call the core function
    process_audio_features(args.input_csv, args.output_csv, args.num_classes)
